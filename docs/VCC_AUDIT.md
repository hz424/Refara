...
The pinned VCC 2026 scoring convention separates model-input controls from
held-out scoring controls and uses a common measured control reference for
predicted and observed effects. This workflow examines how reference assignment
affects supplied predictions' scores. It passes each complete perturbation panel
to the official cell-eval2 implementation and records the cells, predictions and
calibration files used for each result.
...
controls. Participants can document their model inputs and run the public
example; the organizer runs the audit on the held-out panel.
...
format describes what users supply to the adapter. The adapter then assembles
the complete AnnData pair required by the upstream scorer. The reference and
prediction files must contain the same genes and non-control perturbations.
Supply counts that meet the upstream
...
| Record | Purpose |
|---|---|
| Reference artifact and source pool | Identify the scoring matrix, eligible control IDs, dataset and data split |
| Public input pools | Record the controls available to each model at inference |
...
`pred`. Pool/source records in the audit manifest describe the data supplied to
that setting. The upstream scorer has no separate `reference_source` option.
...
The receipt records this assembled scoring input and the original submission
files. The original files and all non-control predictions remain unchanged.
...
Each scored reference view has baseline and replicate calibration files checked
against its inputs and configuration. For a separated-reference arm, the baseline prediction uses the
...
Planning specifies the requested analyses. Preparation validates the H5AD inputs
and writes scoring requests tied to those files. Scoring then runs in the
official environment. Use a fresh output directory when the manifest, input
files or upstream revision change.
...
metric tables, baseline and anchor artifacts, and the summary together. These
records identify the original files, assembled scoring inputs and parameters.
...
the retained component tables show which quantities changed. The identity
`d_D = d_S + V` applies to ordinary MSE under the balanced, equal-depth design
and model-output assumptions in the [MSE guide](REFERENCE_DESIGN.md). It does
not extend to the VCC metric suite.
...
- `truth_informed` draws **Poisson** counts from each selected observed
  perturbation mean. It uses the observed responses to provide a scoring
  example, so its scores do not estimate deployable predictive performance.
...
and the actual input pool is recorded separately. Example preparation checks
the inputs; scoring and independent verification run in separate steps.
