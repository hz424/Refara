...
Use this guide for simulations, training-sensitivity analyses and earlier
benchmark analyses. For the current manuscript results, start with the
...
| Held-out comparison families and input changes | Evaluation-unit and task utilities in separate Source Data, checked against the input manifest | Aggregate command below |
...
Use the `05_SOURCE_DATA` directory with the 1.10.0 input manifest from the matching
...
This reconstructs the 84-comparison held-out family, 56 paired changes in
reference design, the structural-zero correction, sensitivity across repeated
allocations, input changes and separate input/scoring-reference analyses.
It starts from supplied utilities and summary tables. Component guides describe
earlier reconstruction stages and their required inputs.
