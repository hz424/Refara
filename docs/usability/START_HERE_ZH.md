...
用提供的小型合成数据，独立完成数据导入、评分和结果解释。这次练习面向已了解框架、尚未实际操作的项目成员，记录的是首次操作体验。陌生用户的可用性需要另行测试。

任务限时 45 分钟，从环境准备完成后开始计时。安装、查找 Python 环境和阅读数据说明的时间单独记录。可以查阅文档、命令帮助和报错，先尝试独立修改配置并解释结果。遇到问题时，保留报错并记录停在哪一步；如向作者求助，请记录帮助内容，并注明本次练习是在帮助下完成的。
...
每组材料的 `cells.h5ad` 中，一行对应一个细胞。合成数据中，`obs` 的 `patient` 表示独立供体，`treatment` 表示实验状态，`compartment` 表示细胞类型，`lane` 表示采集层。`vehicle` 是对照，`stim` 是处理。只比较 `tasks.tsv` 指定的供体、状态和细胞类型。各供体等权汇总，同一供体内的任务也等权。

使用已完成预处理的 `layers['measured']`。`X` 中是另一套数值，不适用于此次比较。基因通过 `var_names` 匹配。处理组先在每个 lane 内求均值，再对各 lane 的均值等权平均。每个对照 lane 有四个可用细胞，每个对照块从每个 lane 中抽取两个；同一次分配中，不同块不得重复使用细胞。每组任务做两次分配，随机种子设为 19。

`first.h5ad` 与 `second.h5ad` 都是一任务一行，任务名在 `obs['task_name']`，预测数值在 `X`，基因在 `var_names`；行列顺序不保证相同。A、B 中两模型直接预测效应量，没有使用此次分配的对照细胞作为模型输入。要比较的是对新观测效应的预测误差。C 的 first 模型原本预测处理状态，文件中保存的是减去了某个基线的数值；提供者没有交付该基线。C 的 second 仍是直接效应预测。模型来源说明必须保留。
