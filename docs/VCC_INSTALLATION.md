...
The combined package and the official scorer use separate Python environments.
The Python 3.10 environment includes AnnData preparation, ordinary-MSE protocols,
organizer decisions and the VCC adapter:
...
The adapter package requires Python 3.10. Official cell-eval2 requires Python 3.11 or later; this installation route selects Python 3.12, matching the recorded environment. Bash, Git, the two Python executables with `venv` support, and access to GitHub and the Python package index are required. The CPU route uses the explicit `scanpy` DE backend and installs no GPU extras.
...
- `py310/`: the combined package's editable installation, pinned dependencies and AnnData preparation support;
...
Submit with `sbatch setup_vcc.sbatch`. This is an independent CPU job; no GPU allocation or nested `srun` is needed. The script follows `SLURM_CPUS_PER_TASK` for numerical-library thread limits. The recorded parent installation used two CPUs and 12 GB. Choose subsequent scoring resources for the size of your inputs.
...
Keep the test results and scoring outputs together. When every score is available, the summary reports `COMPLETE_DESCRIPTIVE_VCC_AUDIT`. If the official scorer returns a recognized calibration refusal for an allocation, the audit retains all of its model and metric rows and scores the valid allocations. A completed run with such refusals reports `UNAVAILABLE_OFFICIAL_SCORES`, execution status `COMPLETE_WITH_UNAVAILABLE_ANALYSES`, and exit code 2. Missing or failed workers are execution failures. The [audit guide](VCC_AUDIT.md) explains how to distinguish these outcomes.
