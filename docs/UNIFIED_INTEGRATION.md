...
pinned official scorer above. Its nine allocations produced 27 model outcomes:
15 scored and 12 calibration-unavailable, with no failed or missing workers.
...
These checks use fixed teaching predictions and test agreement with the official
scorer. They do not assess predictive performance on hidden VCC contexts. The
VCC calculation uses panel ratios of sums; the MSE protocol averages task scores
with its declared unit weights. Organizer reporting reads supplied score and
geometry tables; automatic conversion from VCC results is not implemented.
