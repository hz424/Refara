...
For Figures 2 and 3 and Extended Data Figure 1, run:

```bash
.venv/bin/python evidence/current_submission/replay.py --output outputs/current_submission
```

This [replay](../evidence/current_submission/README.md) uses the completed
2,560-epoch Influenza CPA fits and reconstructs model comparisons, input changes
and metric concordance from saved scores and summaries for each fitted seed and
evaluation donor. Its manifest checks the inputs against the matching Source Data.
The metric-concordance files retain the historical `ed9/` directory and output
...
Each dataset's decisions are fixed before the evaluation script reads its full
assessment scores. These retrospective comparisons use previously analysed data;
this execution order is not prospective validation. The [organizer guide](organizer.md) describes the commands and inputs;
...
The [replay inputs](../evidence/reporting_reliability/USAGE.txt) supply compressed
task-by-allocation losses, control-role labels and expected results for fixed models.
The replay compares margin, repeated splitting, cross-fitting, bootstrap and Refara.
It reconstructs assessment intervals, the number of comparisons each rule
reports at its own threshold and at matched coverage, sensitivity to control
pools, and inference costs. A separate output records cross-fitting costs for
the first partition.
...
- The [current PBMC replay](../evidence/current_submission/README.md) lists all
  ...
  each group. Figure 3 averages per-seed results and weights the eight evaluation donors equally;
...
and [data access](DATA_ACCESS.md) for external inputs and additional reconstruction files.
