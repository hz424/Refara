...
- `WITHHELD`: the configuration is valid, but independence or training/evaluation
  separation is declared not established, or the comparison has fewer than
  `K_min` non-tied units; `withheld_reason` records the cause.
...
The exact-sign null is that the first model's probability of higher utility is
at most one half at every declared unit position, conditional on availability
and absence of a tie. With independent units, a rejection indicates that this
null fails at at least one position. Equal-unit mean utility differences are
descriptive summaries of a different quantity.
...
The original fixed-configuration replay summarizes 84 unordered contrasts
using max-|T|, Bonferroni-t and marginal-t/Holm diagnostic bounds. Calibration
checks did not establish nominal coverage, so these bounds are retained as
diagnostics rather than calibrated confidence intervals. V1 applies one-sided
exact-sign/Holm tests to 168 ordered comparisons in three 56-direction families.
It audits the supplied unit utilities under that separate rule. The current
manuscript's primary reference comparisons describe the evaluated donors and
control allocations.
