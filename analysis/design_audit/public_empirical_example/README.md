...
The generic CLI and the original fixed-configuration replay perform different
analyses:
...
- `scripts/build_gse162632_public_evidence_v1.py --check` reproduces the
  original 84 unordered within-scheme contrasts and their max-|T|,
  Bonferroni-t and marginal-t/Holm diagnostic bounds. Calibration checks did
  not establish nominal coverage, so the bounds remain diagnostics rather
...
that the roots are independent. The current manuscript's primary reference
comparisons describe the evaluated donors and control allocations; the
84-contrast replay preserves the earlier diagnostic calculations.
