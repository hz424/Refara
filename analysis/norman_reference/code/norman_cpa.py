...
def omitted_r2_metric(*args, **kwargs):
    # Logging-only diagnostic, called after optimizer updates; no RNG/loss use.
    # CPU/GPU traces prove its omission preserves gradients, optimizer and RNG.
    return 0., 0.
...
def fit(adata, condition_key, output, protocol_sha, use_gpu, epochs=EPOCHS, smoke=False, input_sha="synthetic"):
    ...
    model.module.r2_metric = omitted_r2_metric
    ...
    history = history.drop(columns=['r2_mean', 'r2_var'])
    ...
    record = dict(native_output='state', input_scale='whole-transcriptome CP10K-log1p',
        ...
        omitted_logging_metrics=['r2_mean', 'r2_var'],
