"""Deterministic direct-effect baselines used in the GSE306429 analysis.
...
class GSE306429BaselineError(BenchmarkContractError):
    """Raised when a direct baseline receives invalid input or cannot fit."""
...
def _feature_ids(values: Sequence[object]) -> tuple[str, ...]:
    # The caller supplies feature order; this module preserves it.
...
def _finite_matrix(
    ...
    *,
    ...
    if array.ndim != ndim or (0 in array.shape and not allow_empty):
        raise GSE306429BaselineError(
...
def normalized_dose(value: object) -> str:
    """Return the canonical dose spelling used by the predictors."""
...
def compound_dose_token(compound: str, dose_um: object) -> str:
    """Return a categorical compound-dose predictor token."""
...
def _freeze_state_array(
    ...
    *,
    ...
    return _finite_matrix(
        value, name=name, shape=shape, ndim=ndim, allow_empty=allow_empty
    )
...
class PCA64AdditiveRidgeDirectState(_StateMixin):
    """PCA effect representation with an additive latent predictor."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise GSE306429BaselineError("PCA max_rank must equal 64")
        if float(self.ridge_lambda) != RIDGE_LAMBDA:
            raise GSE306429BaselineError("ridge_lambda must equal 1.0")
        ...
        for row in frozen_components:
            ...
            if row[pivot] < 0.0:
                raise GSE306429BaselineError(
                    "PCA component violates the sign rule"
                )
...
class RBFKernelRidgeDirectState(_StateMixin):
    """Unit-bandwidth RBF kernel ridge on metadata predictors."""
    ...
    def __post_init__(self) -> None:
        ...
        if float(self.gamma_factor) != RBF_GAMMA_FACTOR:
            raise GSE306429BaselineError("RBF gamma factor must equal 0.5")
...
def _finite_prediction(values: object, n_rows: int, n_features: int) -> np.ndarray:
    ...
    if prediction.shape != (n_rows, n_features):
        raise GSE306429BaselineError("prediction shape is invalid")
    if not np.isfinite(prediction).all():
        raise GSE306429BaselineError("prediction contains a nonfinite value")
...
def _ridge_with_unpenalized_intercept(
    ...
    """Solve the ridge objective without penalizing the intercept."""
...
def fit_rbf_kernel_ridge_direct(
    *,
    ...
    dose_sd = 0.0 if np.all(log_doses == log_doses[0]) else float(log_doses.std(ddof=1))
    if not math.isfinite(dose_mean) or not math.isfinite(dose_sd):
        raise GSE306429BaselineError("RBF dose standardization failed")
    # Exact constants use the zero-variance rule despite rounding in their mean.
    inputs = _rbf_inputs(
        table.contexts,
        table.compounds,
        table.doses_um,
...
def fit_internal_direct_method(
    ...
    *,
    ...
    """Fit one registered trainable direct-effect method.
    ...
    try:
        ...
        raise GSE306429BaselineError(
            f"unsupported direct-effect method: {method_id!r}"
        ) from error
    return fitter(
        task_ids=task_ids,
        contexts=contexts,
...
def predict_internal_direct(
    ...
    *,
    ...
    """Predict a direct effect without a reference argument."""
    ...
    if not isinstance(
        ...
        raise GSE306429BaselineError("unsupported direct-effect state")
